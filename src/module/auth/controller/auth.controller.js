...
export const userSignup = async (req, res) => {
    try {
        const { userName, email, dateOfBirth, country, password } = req.body;
        const user = await userModel.findOne({ email });
        if (user) {
            return res.status(409).json({ message: "email Already Exist" });
        }
        const hashPass = hashText(password);
                  ...
                  <tr>
                   <td align="left" style="padding:0;Margin:0;width:600px">
                    <table cellpadding="0" cellspacing="0" width="100%" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px">
                      ...
                      <tr>
                       <td align="center" style="padding:0;Margin:0;padding-bottom:35px"><p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, 'helvetica neue', helvetica, sans-serif;line-height:18px;color:#333333;font-size:12px">TourPulse&nbsp;© 2023 TourPulse, Inc. All Rights Reserved.</p></td>
                      </tr>
                      <tr>
                       <td style="padding:0;Margin:0">
                        <table cellpadding="0" cellspacing="0" width="100%" class="es-menu" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px">
...
export const confirmEmail = async (req, res) => {
    ...
    if (!user) {
        return res.status(400).json({
            message: "invalid to verify your email or it's already verified",
        });
    }
...
export const userSignin = async (req, res) => {
    ...
    if (!user.confirmEmail) {
        return res.status(400).json({ message: "confirm email not verified" });
    }
    const match = comparePass(password, user.password);
    if (!match) {
        return res.status(400).json({ message: "invalid data" });
    }
    //if user login but no action requested
